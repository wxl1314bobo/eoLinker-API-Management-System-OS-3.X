...
class AutomatedTestCaseSingleModule
{
     ...
     * @return bool
     */
    public function addSingleTestCase(&$user_id, &$case_id, &$case_data, &$case_code, &$status_code, &$match_type, &$match_rule, &$api_name, &$api_uri, &$api_request_type)
    {
        ...
        $dao = new AutomatedTestCaseSingleDao();
        $result = $dao->addSingleTestCase($case_id, $case_data, $case_code, $status_code, $match_type, $match_rule, $api_name, $api_uri, $api_request_type);
        if ($result) {
            // 刷新项目更新时间
